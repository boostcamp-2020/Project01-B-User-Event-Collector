...
final class LibraryUseCaseTests: XCTestCase {
    ...
    func test_load_tracks_success() {
        let expectation = XCTestExpectation(description: "liked tracks load success test")
        ...
        let usecase = LibraryUseCase(network: MockSuccessNetworkService(data: data!))
    ...
    func test_load_tracks_failure() {
        let expectation = XCTestExpectation(description: "liked tracks load failure test")
        defer { wait(for: [expectation], timeout: 5) }
        
        let usecase = LibraryUseCase(network: MockFailureNetworkService())
